    ...
    : debug (_debug), lossy_percent(0), dst (_dst), ch (0), waiting (false)
{
    ...
    assert(pthread_mutex_init(&send_m, 0) == 0);
...
cchan::~cchan()
{
    ...
    assert(pthread_mutex_destroy(&send_m) == 0); 
...
cchan::done()
{
    ...
    assert(pthread_mutex_lock(&send_m) == 0);
    delete ch;
    ch = 0;
    assert(pthread_mutex_unlock(&send_m) == 0);
...
schan::~schan()
{
    ...
    while (tcpchans.size() > 0) {
        waiting = true;
        if (debug)
            printf("~schan waiting for tcpchan disappear %zd\n", tcpchans.size());
    ...
    // It's only safe to destroy send_m when all dispatched threads are
    // finished and done() is complete.  rpcs deconstructor
    // waits for dispatched threads to finish before invoking this deconstructor
    // and tcpchans_c is signalled after done() completes.
