    ...
    : dst(_dst), debug(_debug), xid(1), svr_nonce(0), bind_done(false), chan(_dst, _debug), _vivaldi(NULL), destroy_wait (false)
{
    ...
    srandom(tv.tv_sec ^ (tv.tv_usec << 12) ^ (getpid() << 4));
    clt_nonce = random();
...
// This function is required for operation in a VM where two
// successive gettimeofday() calls are not guaranteed to be
// increasing.
int
timeval_subtract_positive (struct timeval *result, struct timeval *x, struct timeval *y)
{
    int ret = timeval_subtract(result, x, y);
    if (ret == 1) {
        // Make the smallest positive difference.
        result->tv_sec = 0;
        result->tv_usec = 1;
        ret = 0;
    }
...
rpcc::call1(unsigned int proc, const marshall &req, unmarshall &rep, TO to)
{
    ...
    while(1){
        if(ca.done)
            break;
        assert(timeval_subtract_positive(&diff, &now, &last) == 0); // must be positive 
...
// For LAB 1: Understand what this code is doing: 
// What invariant does this function try to maintain for xid_rep_window?
// What does xid_rep_window.front() represent?
// This should help you understand what RPCs you can 'forget about'.
void
...
rpcs::dispatch(junk *j)
{
    ...
    if(!req.ok() || procs.count(proc) < 1){
        assert(pthread_mutex_unlock(&procs_m) == 0);
        if (debug) printf("bad proc %x req ok? %d procs.count %zd\n",
...
rpcs::rpcstate_t
rpcs::checkduplicate_and_update(unsigned int clt_nonce,
